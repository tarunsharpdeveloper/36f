...
if (!checkAccessLevel($_SESSION['user']['access_level'], 'locations', 'add')) {
   _R('location');  
}

if (isset($_REQUEST['ajaxSubmitAll'])) {

    $data = array();
    parse_str($_REQUEST['ladelData'], $data);
    $areawork_values_array = $data['area'];
    $name_values_array = $data['name'];
...
//    $fields['timezone'] = $data['locTimeZone'];
    $st = qi("tb_location", $fields);

    $loc = qs("select * from tb_location where name='{$data['locName']}' and company_id={$_SESSION['company']['id']}");
    if (!empty($areawork_values_array)) {
        $fields = array();
        for ($i = 0; $i < count($areawork_values_array); $i++) {

            $fields['company_id'] = $_SESSION['company']['id'];
            $fields['location_id'] = $loc['id'];
            $fields['area_work'] = _escape($areawork_values_array[$i]);
            qi('tb_area_work', _escapeArray($fields));
        }
    }
...
//        $fields['work_at'] = $_SESSION['company']['id'];
...
//    d($_REQUEST);
    $fields = array();
    $fields['company_id'] = $_SESSION['company']['id'];
    $fields['name'] = $_REQUEST['locName'];
    $fields['address'] = $_REQUEST['locAddress'];
    $fields['week_starton'] = $_REQUEST['locWeekStart'];
    $fields['latlng'] = $_REQUEST['latlang'];
    $fields['timezone'] = $_REQUEST['locTimeZone'];
    $st = qi("tb_location", $fields);
    if ($st >= 1) {
        _R('location');
    } else {
...
$CompanyWork = q("select * from tb_company_works");
$StreetProfile = q("select * from tb_stress_profile");
$jsInclude = 'add_location.js.php';
_cg("page_title", "New Location");
