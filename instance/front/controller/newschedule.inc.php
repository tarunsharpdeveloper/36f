...
//    _R('login');
//}
if (isset($_REQUEST['newschedule_submit'])) {
//    d($_REQUEST);
    $title = $_REQUEST['title'];
    $start = date("Y-m-d H:i:s", strtotime($_REQUEST['dateofshift']));
    $end = date("Y-m-d H:i:s", strtotime($_REQUEST['enddateofshift']));
//    $start = $_REQUEST['dateofshift'];
//    $end = $_REQUEST['enddateofshift'];
    $url = "";
    $emplist = $_REQUEST['emplist'];
    $notes = $_REQUEST['md_txtnotes'];
// connection to the database
    ...
    $fields['end'] = $end;
    $fields['url'] = $url;
    $fields['emp_id'] = $emplist;
    $fields['notes'] = _escapeArray($notes);
...
if (isset($_REQUEST['getDayViseData'])) {
    $UserId = $_SESSION['user']['id'];
    $selecetdDate = date('Y-m-d', strtotime($_REQUEST['selectedDate']));

    $task = q("select * from tb_task where assign_to='$UserId' and due_date='$selecetdDate'");
//    d($_REQUEST);
//    d($task);
...
//    d($weeks);
//    d($weeks_month);
    echo json_encode(array("weeks" => $weeks, "weeks_month" => $weeks_month, "dayno" => $weeks_day, "dayname" => $weeks_Day));
    die;
}
